       ...
       <header className='bg-gradient-to-r from-pink-100 to-purple-100 sticky top-0 z-50'>
        <div className='max-w-screen-xl mx-auto px-4 py-4 flex justify-between items-center'>
                    ...
                    <nav aria-label="Mobile navigation">
                        <ul className="flex flex-col space-y-4">
                            {navLinks.map((link, index) =>(
                                <li key={index}>
                                    <a href={link.ref}
                                    className="block text-gray-700 hover:text-pink-600 transition-colors duration-200 py-2 px-2 focus:outline-none focus:ring-2 focus:ring-pink-600 focus:rounded"
                                    onClick={toggleMenu}>
                                        {link.label}
                                    </a>
                                </li>
            ...
            <nav className="hidden md:block" aria-label="Appointment booking">
                <a href="#appointment"
                className="bg-pink-600 hover:bg-pink-700 text-white px-6 py-2 rounded-full transition-colors duration-200 shadow-md focus:outline-none focus:right-2 focus:ring-pink-800 focus:ring-offset-2">
                    Book Appointment
                </a>
            </nav>
